 ...
 * @returns True if it's a workday
 */
...
/**
 * Seeds the database with comprehensive mock data
 * @description Creates users, clients, projects, tasks, assignments, time entries, and absences
 */
export const seedDatabase = async (): Promise<void> => {
    try {
        ...
        // Each task goes to exactly one employee (exclusive assignment)
        ...
        for (let empIndex = 0; empIndex < employees.length; empIndex++) {
            ...
            // Ensure each employee has at least 8 tasks minimum
        ...
        logger.info(`   - Users: 1 admin + ${employees.length} employees = ${employees.length + 1} total`);
